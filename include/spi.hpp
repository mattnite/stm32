...
namespace Spi {
    ...
    template <typename Peripheral, typename DataFrame, Type type,
              auto lsb_first = false, auto mode = 0,
              typename = std::enable_if_t<sizeof(DataFrame) == 1 ||
                                          sizeof(DataFrame) == 2>>
    struct Module {
        Module() {
...
struct Reset<Mcu, typename Mcu::SPI1> {
    using Field = typename Mcu::RCC::APB2RSTR::SPI1RST;
};

template <typename Mcu>
struct Reset<Mcu, typename Mcu::SPI2> {
    using Field = typename Mcu::RCC::APB1RSTR::SPI2RST;
};

template <typename Mcu>
struct ClockEnable<Mcu, typename Mcu::SPI1> {
    using Field = typename Mcu::RCC::APB2ENR::SPI1EN;
};

template <typename Mcu>
struct ClockEnable<Mcu, typename Mcu::SPI2> {
    using Field = typename Mcu::RCC::APB1ENR::SPI2EN;
};

template <typename Mcu>
struct SleepEnable<Mcu, typename Mcu::SPI1> {
...
struct SPI1 {
    ...
    struct I2SCFGR : public Register<0x4001301c> {
        ...
        using CHLEN =
            Field<0, 1>; // Channel length (number of bits per audio channel)
    };
